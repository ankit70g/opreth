...
export const getFilePath = query({
    ...
    handler: async (ctx, args) => {
        ...
        while (currentId) {
            const file = (await ctx.db.get("files", currentId)) as | Doc<"files">
            | undefined;
            if (!file) break;
...
export const createFile = mutation({
    ...
    handler: async (ctx, args) => {
        ...
        // Check if the file with same name already exists in this parent folder
        const files = await ctx.db
            .query("files")
            .withIndex("by_project_parent", (q) => q.eq("projectId", args.projectId).eq("parentId", args.parentId))
            .collect();
...
export const createFolder = mutation({
    ...
    handler: async (ctx, args) => {
        ...
        // Check if the folder with same name already exists in this parent folder
        const files = await ctx.db
            .query("files")
            .withIndex("by_project_parent", (q) => q.eq("projectId", args.projectId).eq("parentId", args.parentId))
            .collect();
...
export const renameFile = mutation({
    ...
    handler: async (ctx, args) => {
        ...
        if (!project) {
            throw new Error("Project not fpund");
        }
