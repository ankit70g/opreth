...
export const exportToGithub = inngest.createFunction(
  {
    ...
    onFailure: async ({ event, step }) => {
      const internalKey = process.env.OPRETH_CONVEX_INTERNAL_KEY;
  ...
  async ({ event, step }) => {
    ...
    const internalKey = process.env.OPRETH_CONVEX_INTERNAL_KEY;
    if (!internalKey) {
      throw new NonRetriableError("OPRETH_CONVEX_INTERNAL_KEY is not configured");
    };
    ...
    const { data: repo } = await step.run("create-repo", async () => {
      return await octokit.rest.repos.createForAuthenticatedUser({
        name: repoName,
        description: description || `Exported from Opreth`,
    ...
    const { data: commit } = await step.run("create-commit", async () => {
      return await octokit.rest.git.createCommit({
        owner: user.login,
        repo: repoName,
        message: "Initial commit from Opreth",
        tree: tree.sha,
